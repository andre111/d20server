...
public class PropertyVariableActor extends PropertyVariable {
	...
	protected Property getProperty(Map map, Profile profile) throws ScriptException {
		// get property
		Property property = actorFinder.findActor(map, profile).getProperty(propertyName);
		if(property == null) {
	...
	protected Access getAccessLevel(Map map, Profile profile) throws ScriptException {
		return actorFinder.findActor(map, profile).getAccessLevel(profile);
	}

	@Override
	protected void saveSourceAfterSet(Map map, Profile profile) throws ScriptException {
		Actor actor = actorFinder.findActor(map, profile);
