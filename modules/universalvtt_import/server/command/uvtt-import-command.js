...
export class UVTTImportCommand extends Command {
    ...
    execute(profile, args) {
        ...
        if(splitArgs.length != 3) throw `Usage: /${this.getName()} <filePath> <name> <importLights>`;

        // check filePath
        const filePath = './data/files'+splitArgs[0];
        if(filePath.includes('..')) throw 'Invalid path'; //TODO: create a general reusable (and known safe) method for validating paths (to also use in file manager)!
        ...
        if(!data.format || !data.resolution || !data.line_of_sight || !data.portals || !data.environment || !data.lights || !data.image) {
            throw 'File does not appear to be an Universal VTT file';
        }
