class CanvasRenderLayerMeasurements extends CanvasRenderLayer {
    constructor(level) {
        super();
    ...
    render(ctx, view, viewers, camera, viewport, map) {
        ...
        for (var [profile, measurement] of pfMeasurements) {
            if(measurement.map != ServerData.currentMap.get()) continue;
            
            var color = "#" + (ServerData.profiles.get().get(profile).color & 0x00FFFFFF).toString(16).padStart(6, '0');
            ctx.fillStyle = color;
