	...
	preloadSounds(){
		...
		for (const [surface, numsounds] of surfaces) {
			...
			for (let s=1; s <= numsounds; ++s) {
				let path = `/public/3ddice/sounds/${surface}/surface_${surface}${s}.wav`;
				new Howl({
					src:path,
					autoplay:false
				});
		...
		for (const [material, numsounds] of materials) {
			...
			for (let s=1; s <= numsounds; ++s) {
				let path = `/public/3ddice/sounds/dicehit/dicehit${s}_${material}.wav`;
				new Howl({
					src:path,
					autoplay:false
				});
		...
		for (let i=1; i <= 6; ++i) {
			let path = `/public/3ddice/sounds/dicehit/coinhit${i}.wav`;
			new Howl({
				src:path,
				autoplay:false
			});
	...
	loadContextScopedTextures(type){
		return new Promise(resolve => {
			...
			if(this.dicefactory.bumpMapping){
				...
				this.renderer.scopedTextureCache.roughnessMap_fingerprint = textureLoader.load('/public/3ddice/textures/roughnessMap_finger.webp');
				this.renderer.scopedTextureCache.roughnessMap_wood = textureLoader.load('/public/3ddice/textures/roughnessMap_wood.webp');
				this.renderer.scopedTextureCache.roughnessMap_metal = textureLoader.load('/public/3ddice/textures/roughnessMap_metal.webp');
				...
				.setPath( '/public/3ddice/textures/equirectangular/' )
				...
				loader.setPath('/public/3ddice/textures/cubemap/');
